...
@section('content')
    ...
    <form action="{{ route('tasks.store') }}" method="POST" class="bg-white border border-[#DDD5C7] rounded-2xl p-4 mb-8 space-y-3">
        ...
        <input type="text" name="title" placeholder="Nova tarefa..." required
            class="w-full bg-[#F4F0E8] border border-[#DDD5C7] rounded-xl px-4 py-3 text-[15px] focus:outline-none focus:border-[#6B7A5E]">
        <div class="flex gap-2">
            <input type="text" name="tags" placeholder="tags (opcional)"
                class="flex-1 bg-[#F4F0E8] border border-[#DDD5C7] rounded-xl px-4 py-3 text-[15px] focus:outline-none focus:border-[#6B7A5E]">
            <select name="type" class="bg-[#F4F0E8] border border-[#DDD5C7] rounded-xl px-3 py-3 text-[15px]">
    ...
    <h2 class="text-xs font-semibold text-[#8A8171] mb-3 uppercase tracking-wide">Abertas ({{ $tasksAbertas->count() }})</h2>
    <ul class="space-y-2 mb-10">
            ...
            <li>
                <a href="{{ route('ritual.form', $task) }}" class="flex items-center justify-between bg-white border border-[#DDD5C7] rounded-2xl px-4 py-4 active:bg-[#F4F0E8]">
                    <div class="min-w-0">
                        ...
                        <p class="text-xs text-[#A79E8C] mt-0.5 uppercase">
                            {{ $task->type }}
                            @if ($task->tags)
        ...
        @empty
            <li class="text-[#A79E8C] text-sm py-4 text-center">Nenhuma tarefa aberta.</li>
        @endforelse
    </ul>

    <h2 class="text-xs font-semibold text-[#8A8171] mb-3 uppercase tracking-wide">Encerradas recentemente</h2>
    <ul class="space-y-1">
        @foreach ($tasksEncerradas as $task)
            <li class="text-sm text-[#A79E8C] line-through px-1">{{ $task->title }}</li>
        @endforeach
    </ul>
@endsection
