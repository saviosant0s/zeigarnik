...
@section('content')
    <a href="{{ route('ritual.index') }}" class="inline-flex items-center gap-1 text-xs text-[var(--text-faint)] mb-3 mt-2">
    ...
    <form action="{{ route('ritual.save', $task) }}" method="POST" class="bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-4 space-y-4">
        @csrf
        <div>
